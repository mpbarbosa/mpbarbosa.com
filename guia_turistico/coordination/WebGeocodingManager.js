...
class WebGeocodingManager {
	...
	constructor(document, params) {
		...
		// Create fetch manager - try to use IbiraAPIFetchManager if available
	...
	_wireObservers() {
		...
		// Note: These are commented out in original implementation
	...
	_handleFindRestaurantsClick() {
		if (this.currentCoords) {
			// Delegate to external function if available
			if (typeof window !== 'undefined' && typeof window.findNearbyRestaurants === 'function') {
				window.findNearbyRestaurants(
					this.currentCoords.latitude,
					this.currentCoords.longitude
				);
			} else {
				// Fallback implementation
				alert(`Procurando restaurantes próximos a ${this.currentCoords.latitude.toFixed(4)}, ${this.currentCoords.longitude.toFixed(4)}`);
			}
		} else {
	...
	_handleCityStatsClick() {
		if (this.currentCoords) {
			...
			if (typeof window !== 'undefined' && typeof window.fetchCityStatistics === 'function') {
				window.fetchCityStatistics(
					this.currentCoords.latitude,
					this.currentCoords.longitude
				);
			} else {
				// Fallback implementation
				alert(`Obtendo estatísticas da cidade para ${this.currentCoords.latitude.toFixed(4)}, ${this.currentCoords.longitude.toFixed(4)}`);
			}
		} else {
			alert("Current coordinates not available.");
		}
